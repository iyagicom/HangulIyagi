...
| 설정 | 설명 | 기본값 |
|------|------|--------|
| 후보 단어 창 | 타이핑 중 후보 표시 ON/OFF | OFF |
...
| **영→한 자동변환** | 영문 타이핑 후 전환 시 한글로 자동 변환 | OFF |
...
`hanguliyagi_1.6.0_amd64.deb` 파일을 **파일 관리자에서 더블클릭**하면 설치된다.
...
```bash
unzip hangulIyagi-v1.6.0-linux-x64.zip
cd hangulIyagi-v1.6.0-linux-x64
./install.sh
```
